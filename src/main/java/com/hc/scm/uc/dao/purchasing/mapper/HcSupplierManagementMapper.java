...
import com.hc.scm.uc.dao.purchasing.provider.HcSupplierSelectParamterProvider;
import com.hc.scm.uc.dao.purchasing.provider.HcSupplierUpdateProvider;
import com.hc.scm.uc.dao.selectprovider.ItgUserRoleSelectProvider;
...
public interface HcSupplierManagementMapper extends BaseCrudMapper {
	 @Select("SELECT  [co_number],[coname],[coaddr],[copost],[city],[country],[province],[cofrdb],[cozzxs],[cozczb]"
	 		...
	 		+ ",[qlty_control],[companymt],[scale],[warehouse],[describee] ,(CASE WHEN province=city THEN city ELSE province+city END)  as provincesandcities  FROM [supplier]"
	 		+ "  where  1=1  order by rg_date desc ")
	 @Results(
			 {
				 ...
				 , @Result(column="valueco", property="valueCategory")//价值类别
	 ...
	 @Insert("INSERT INTO [supplier]([co_number],[coname],[coaddr],[copost],[city],[country],[province],[cofrdb],[cozzxs],[cozczb],[coyyzz],[cotypes]" +
	 		",[tradetypes],[cokhjb],[cokhyh],[coyhzh],[coclrq],[ifjckq],[cotel],[cofax],[codzyj],[conet],[cosyhb],[cojsfs],[nshm],[number],[username]" +
	 		",[dept],[deptjb] ,[modman],[mod_date],[share],[yearearning],[valueco] ,[rg_date],[annual_sales],[sales_ability],[qlty_control],[companymt],[scale]" +
	 		",[warehouse],[describee] ,[in_no])" +
	 		" VALUES" +
	 		"(#{companyNo},#{companyName},#{companyAddr},#{companyZipCode},#{city},#{country},#{province},#{legalRepresentative},#{organizationForm} ,#{registeredCapital},#{supplierTypes}" +
	 		" ,#{industryTypes},'',#{supplierLevel},#{bankOfDeposit},#{bankAccount},#{dateOfEstablishment},#{paymentMethod},#{companyTel},#{companyFax},#{email},#{companyNet},#{useCurrency},#{linkMan}" +
	 		",#{linkManTel},#{docPurchasing},#{createUser},#{localDepartment},'',#{modifyUser},#{modifyDate},#{whetherToShare},#{yearEarning},#{valueCategory},#{loginDate},#{companyScale}" +
	 		",#{abilityToSell},#{qualityControl},#{companyProfile},#{categoryOfTheProduct} ,#{cooperationOpportunities},#{describee},2)") 
	 public <ModelType> int insertSelective(ModelType record);
	 
	 @UpdateProvider(type = HcSupplierUpdateProvider.class, method = "updateSql")
	 public <ModelType> int updateByPrimaryKeySelective(ModelType record);

	 
	 @Delete("DELETE FROM supplier WHERE co_number=#{companyNo}")
	 public <ModelType> int deleteByPrimarayKeyForModel(ModelType record);
	 
	 
	 @SelectProvider(type=HcSupplierSelectParamterProvider.class, method = "findSelecttByParamsSql")
	 @Results(
			 {
				 @Result(id=true,column="co_number", property="companyNo")  //供应商编码
				 , @Result(column="coname", property="companyName")  //供应商名称
	 ...
	 public  List<HcSupplier> selectByPage(@Param("orderByField") String orderByField,@Param("orderBy") String orderBy,@Param("params")Map<String,Object> params);
